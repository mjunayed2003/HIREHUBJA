...
export default function EmployerProfile() {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      {/* Banner */}
      <div className="relative w-full h-[250px] overflow-hidden rounded-b-xl">
        <Image 
          src="/image/handshake-banner.jpg" 
          alt="Banner" 
          fill 
      ...
      {/* User Info Section */}
      <div className="px-10 -mt-20 relative z-10 flex items-end gap-6">
        <div className="w-44 h-44 rounded-2xl border-4 border-white overflow-hidden bg-white shadow-lg">
          <Image src="/image/avatar.jpg" alt="Profile" width={176} height={176} className="object-cover" />
        </div>
        <div className="pb-4">
      ...
      <div className="max-w-2xl mx-auto mt-10 space-y-4">
        <ProfileListItem 
          ...
          onClick={() => router.push('/auth/employer/professional-details')} 
        />
        <ProfileListItem 
          icon={<Settings className="text-white" size={18}/>} 
          label="Settings" 
          onClick={() => router.push('/auth/employer/settings')} 
        />
        <ProfileListItem 
...
function ProfileListItem({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-4">
        <div className="w-9 h-9 bg-[#3FAE2A] rounded-full flex items-center justify-center">
          {icon}
        </div>
